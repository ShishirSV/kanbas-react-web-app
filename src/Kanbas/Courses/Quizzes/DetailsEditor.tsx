...
import { fetchQuizDetails, updateQuizDetails, createQuiz } from './client';
...
function QuizEditor() {
  ...
  useEffect(() => {
    if (cid && qid !== "new" && !quiz._id) {
      fetchQuizDetails(cid, qid!).then(details => {
  ...
  const handleSave = async () => {
    if (qid === 'new') {
      await createQuiz(cid as string, quizDetails);
      navigate(`/Kanbas/Courses/${cid}/Quizzes`);
      console.log('Creating new quiz:', quizDetails);
    }
    else if (qid) {
  ...
  return (
    <div className="col-12" style={{
      ...
      <div className="float-end">
        ...
        <button onClick={handleSave} className="btn btn-danger ms-2">{qid !== "new" ? 'Update' : 'Create'} & Save</button>
      </div>
    </div>
  );
}
